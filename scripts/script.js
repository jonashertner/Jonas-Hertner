...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Drawing Functionality with Paper.js
    const canvas = document.querySelector('.drawing-canvas');
    paper.setup(canvas);

    let path; // Current drawing path
    ...
    canvas.addEventListener('pointermove', (event) => {
        ...
        const point = new paper.Point(event.clientX, event.clientY);
        path.add(point);
        path.smooth(); // Smooth the path for better drawing
    });
    ...
    // let isDrawing = false;
    // let currentPath;
    // const pencilColor = '#000'; // Pencil color
    ...
    //     currentPath.setAttribute('stroke', pencilColor); // Set pencil color
